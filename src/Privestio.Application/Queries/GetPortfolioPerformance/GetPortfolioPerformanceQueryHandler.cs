        ...
        return new PortfolioPerformanceResponse
        {
            ...
            Holdings = result
                .Holdings.Select(h => new HoldingPerformanceResponse
                {
                    ...
                    Currency = h.Currency,
                    QuoteCurrency =
                        fxByHoldingId.GetValueOrDefault(h.HoldingId)?.QuoteCurrency ?? h.Currency,
                    IsFxConverted =
                        fxByHoldingId.GetValueOrDefault(h.HoldingId)?.IsFxConverted ?? false,
                    FxRateToAccountCurrency = fxByHoldingId
                        .GetValueOrDefault(h.HoldingId)
                        ?.FxRateToAccountCurrency,
