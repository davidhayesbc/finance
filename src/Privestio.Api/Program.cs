...
try
{
    ...
    if (
        ...
        throw new InvalidOperationException(
            "Jwt:Key must not use the default placeholder value in non-development environments."
        );
    ...
    if (builder.Configuration["Authentication:Microsoft:ClientId"] is { Length: > 0 } msClientId)
    {
        builder
            ...
            .AddOpenIdConnect(
                ...
                {
                    options.Authority =
                        $"https://login.microsoftonline.com/{builder.Configuration["Authentication:Microsoft:TenantId"]}/v2.0";
